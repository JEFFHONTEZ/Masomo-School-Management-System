                ...
                <table class="table datatable-button-html5-columns table-responsive">
                    ...
                    @foreach($uncleared as $uc)
                        <tr>
                            ...
                            {{--Pay Now Form--}}
                            <td>
                                <form id="{{ Qs::hash($uc->id) }}" method="post" class="ajax-pay" action="{{ route('payments.mpesa') }}">
                                    @csrf
                                    <input type="hidden" name="pr_id" value="{{ Qs::hash($uc->id) }}">
                                    <input type="hidden" name="phone" value="{{ $uc->student->parent->phone ?? '' }}">
                                    <div class="row">
                                        <div class="col-md-7">
                                            <input min="1" max="{{ $uc->balance ?: $uc->payment->amount }}" id="val-{{ Qs::hash($uc->id) }}" class="form-control" required placeholder="Pay Now" title="Pay Now" name="amount" type="number">
                                        </div>
                                        <div class="col-md-5">
                                            <button data-text="Pay" class="btn btn-success" type="submit">Pay <i class="icon-paperplane ml-2"></i></button>
                                        </div>
                                    </div>
                                </form>
                            </td>
                                ...
                                <div class="list-icons">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-left">
                                            ...
                                            {{--Reset Payment--}}
{{-- Only show Reset Payment to non-parents --}}
@unless(auth()->user()->user_type == 'parent')
